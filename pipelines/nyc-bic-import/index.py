...
def detect_boro(city: str) -> str | None:
    """Map a NYC city/borough name to its canonical borough label."""
    city_upper = city.upper().strip()
    if city_upper in ("BRONX", "THE BRONX"):
        return "Bronx"
    if city_upper == "BROOKLYN":
        return "Brooklyn"
    if city_upper in ("MANHATTAN", "NEW YORK", "NEW YORK CITY"):
        return "Manhattan"
    if city_upper == "QUEENS":
        return "Queens"
    if city_upper == "STATEN ISLAND":
...
def main() -> None:
    ...
    for rec in license_records:
        ...
        if bic_number:
            license_metadata["bic_number"] = bic_number
        boro = detect_boro(city)
        if boro:
