...
const states = [
  ...
  },
  {
    ...
    slug: "rhode-island",
    abbr: "RI",
    description: "Serving the Ocean State",
  },
  {
    name: "Vermont",
    slug: "vermont",
            ...
            {states.map((state) => (
              <Link
                ...
                className={`group bg-white rounded-xl border border-gray-200 p-6 flex flex-col items-center text-center transition-all ${
                  (state as { comingSoon?: boolean }).comingSoon
              ...
              >
                <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-4 transition-colors ${
                  ...
                  <MapPin className={`size-6 ${(state as { comingSoon?: boolean }).comingSoon ? "text-gray-400" : "text-[#2D6A4F]"}`} />
                </div>
